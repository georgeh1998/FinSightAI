...
フォルダ構成は `input/` 直下だけでなく、サブディレクトリもサポートしています。
証券コードごとにフォルダを分ける運用も可能です。

**配置例:**
- `input/8035_2024_1Q.pdf`
- `input/8035/8035_2024_1Q.pdf`
...
分析が完了すると、 `output/` ディレクトリ内に以下のフォルダ構成でレポートが出力されます。

`output/{証券コード}_{企業名}/{日付}/`

**出力ファイル:**
- `report.md`: 分析レポート（Markdown形式）
- `current_extracted_text.txt`: 今回の決算短信から抽出されたテキスト原文
- `previous_extracted_text.txt`: 前年同期の決算短信から抽出されたテキスト原文（存在する場合）

**出力例:** `output/8035_東京エレクトロン株式会社/2024-01-01/report.md`
